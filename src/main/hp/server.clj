(ns hp.server
  (:require [hp.parser :refer [api-parser]]
...
(defonce
  ^{:doc
      "When we create the websocket we throw it in this atom, so we can use it for other things"}
  *websocket
  (atom nil))
...
(defn start
  ...
  (let [websockets (fws/start! (fws/make-websockets
        ...
        middleware (-> not-found-handler
                       ...
                       wrap-content-type
                       wrap-not-modified)
        server-stop-fn (http/run-server middleware
                                        {:host "0.0.0.0" :port 3000})]
    (reset! *websocket websockets)
    (reset! stop-fn (fn [] (fws/stop! websockets) (server-stop-fn)))))

(defn stop [] (when @stop-fn (@stop-fn) (reset! stop-fn nil)))
